			  ...
			  int pkt_entry_cnt, uint64_t flags)
{
	...
	for (pkt_idx = 0; pkt_idx < pkt_entry_cnt; ++pkt_idx) {
		...
		if (use_inline) {
			...
			sg_list[0].lkey = ((struct efa_mr *)pkt_entry->mr)->lkey; /* provider-owned, never user-closed */
			if (pkt_entry->payload) {
				iov_cnt = 2;
			  ...
			  int pke_cnt)
{
	...
	for (i = 0; i < pke_cnt; ++i) {
		...
		recv_wr->wr.sg_list[0].lkey = ((struct efa_mr *) pke_vec[i]->mr)->lkey; /* provider-owned, never user-closed */
