...
void test_efa_rdm_mr_gen_check_ope_detects_closed_mr(struct efa_resource **state)
{
	...
	/* Before close: gen check should pass */
	assert_int_equal(efa_rdm_mr_gen_check_ope(txe), 0);

	/* Close MR (bumps gen) */
	assert_int_equal(fi_close(&mr->fid), 0);

	/* After close: gen check should detect mismatch */
	assert_int_equal(efa_rdm_mr_gen_check_ope(txe), -FI_ECANCELED);
